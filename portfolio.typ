...
#portfolio-work-detail(
  ...
  루트 시뮬레이터는 최적의 아이템 파밍 경로를 찾아주는 도구입니다. 이터널 리턴은 특정 지역에서 나오는 재료 아이템을 조합하여 더 강한 아이템을 만들어 싸워야 하는 게임인데, 당시 플레이어들은 지역에서 나오는 아이템 목록만을 가지고 스스로 실험하여 루트 경우의 수와 최적의 루트를 찾아야 하는 불편함이 있었습니다. 이러한 불편함을 해결하기 위해 개발하게 되었습니다.\
  ...
  #work-detail(
    ...
    description: [루트 시뮬레이터는 게임 지도 위에 각 지역을 image map 엘리먼트(\<map>, \<area>)로 표현하고, 시뮬레이션된 경로를 svg 엘리먼트로 표현합니다.
...
#portfolio-work-detail(
  ...
  #work-detail(
    ...
    description: [fetch한 데이터를 관리하는 여러 방법론 중 당시 문서와 커뮤니티 자료가 잘 갖춰진 솔루션으로는 SWR과 React Query가 있었습니다. 두 프로젝트는 거의 비슷한 API를 제공하지만, 후자를를 선택했습니다. 그 이유는 다음과 같습니다.
...
#portfolio-work-detail(
  ...
  #work-detail(
    ...
    description: [v1에서 v2로 넘어가면서 Next.js에서 SvelteKit으로 스택을 바꿔 작업하게 되었는데, 국제화 관련 코드를 작성할 때의 불편한 점을 개선하고자 했습니다. 이때 다른 여러 솔루션 중에서 typesafe-i18n을 고르게 되었는데, 이 라이브러리를 선정하게 된 이유는 다음과 같습니다.
      - 더 작은 번들 사이즈(5.6kB gzipped vs 1.3kB gzipped)
      - 여러 언어 파일 사이에 누락된 키가 있는 것을 타입 단계에서 감지하여 특정 언어에서 번역이 되지 않은 문자열이 노출되는 것을 방지
      - 번역 키에 대해 타입 힌트가 제공되어 개발 속도 향상
      - 코드젠 방식으로 타입을 생성하기 때문에 복잡한 타입을 통해 타입 안정성을 제공하는 것에 비해 개발 단계에서 타입스크립트 컴파일러의 부담이 적을 것이라 판단
    ],
  )
]
...
#portfolio-work-detail(
  ...
  #work-detail(
    ...
    description: [
      리팩토링 과정 중 비디오 효과 처리를 위한 WebGL 필터 구현에 결함이 있는 것을 발견하였습니다. 변경이 없어도 매번 셰이더를 컴파일하고 있었고, 각 비디오마다 WebGLContext를 들고 있는 형태였기 때문에 동시 렌더링 수에 제한도 있었습니다. 작업의 큰 카테고리는 리팩토링이었으나 근본적으로는 버그 수정을 겸하고 있었기 때문에, 수정하는 쪽으로 작업 방향을 설정했습니다. 코드를 통해 짐작되는 문제를 바로 수정하는 대신 디버거를 통해 실제 성능 병목이 어딘지 분석하는 과정을 거쳤습니다. 추상화를 포함한 기본 설계부터 다시 진행한 결과
...
#portfolio-work-detail([Web Worker를 사용한 성능 향상 실험], (tech-stacks.typescript, "WebGL", "Web Worker"))[
  ...
  #work-detail(
    ...
    description: [collider가 장착된 엔티티의 수가 1500개를 넘어가면 퍼포먼스가 급격하게 저하되는 문제가 있었습니다. O(N#super([2])) 방식의 나이브한 충돌처리 대신 Spatial Hash나 Quadtree 등을 사용하여 충돌처리를 최적화하는 방식을 고민하게 되었고, 여기에서는 embarrassingly parallel하여 병렬화하기 쉬운 Spatial Hash 방식을 채택하여 실험했습니다. Spatial Hash는 화면을 특정 크기의 cell로 나눠 그 안에 들어있는 객체들과 경계에 걸쳐 있는 객체들만 체크하는 기법입니다.
      흐름을 파악하기 위한 핵심 코드는 다음와 같습니다.
